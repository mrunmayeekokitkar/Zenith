...
export function GlobeClient() {
  ...
  const selectedDetails = useMemo(() => {
    ...
    } else if (Math.abs(lat - -22.9068) < 1.5 && Math.abs(lng - -67.9290) < 1.5) {
      spaceRelevance = "Atacama Desert: Ultra-dry atmosphere, high elevation, absolute minimum light pollution stargazer preset.";
    } else if (Math.abs(lat - 78.2232) < 1.5 && Math.abs(lng - 15.6267) < 1.5) {
      spaceRelevance = "Svalbard: Polar orbit downlink visibility, auroral auroris storm zone preset.";
    } else if (Math.abs(lat - 23.4162) < 2.0 && Math.abs(lng - 25.6628) < 2.0) {
      spaceRelevance = "Sahara Desert: Zero cloud canopy thermal pocket stargazer preset.";
    } else if (Math.abs(lat - 0.0000) < 2.0 && Math.abs(lng - -170.0000) < 2.0) {
      spaceRelevance = "Pacific Ocean: Low altitude water mirror stargazer preset.";
  ...
  const handleFlyToLandmark = useCallback((landmark: Landmark) => {
    ...
    viewer.camera.lookAtTransform(Cesium.Matrix4.IDENTITY);
    viewer.camera.flyTo({
      destination: Cesium.Cartesian3.fromDegrees(landmark.longitude, landmark.latitude, 12000),
      orientation: {
        heading: Cesium.Math.toRadians(0),
  ...
  return (
    <main className="page-with-nav relative h-[100dvh] w-full overflow-hidden bg-[#03040a] pt-20">
      ...
      <button
        ...
        className="lg:hidden pointer-events-auto fixed bottom-6 right-4 z-[60] flex h-12 w-12 items-center justify-center rounded-full border border-white/20 bg-slate-950/80 backdrop-blur-xl text-slate-200 shadow-xl cursor-pointer transition-all hover:border-sky-400/30"
      >
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="animate-[spin_8s_linear_infinite]">
